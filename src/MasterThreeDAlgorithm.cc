...
StatusCode MasterThreeDAlgorithm::GetVolumeIdToHitListMap(VolumeIdToHitListMap &volumeIdToHitListMap) const
{
    ...
    for (const auto &hitTypeToStatusMapEntry : hitTypeToStatusMap)
    {
        ...
        const unsigned int nHitsInTPC(hitTypeToStatusMapEntry.second.first);
        const unsigned int nHitsOutOfTPC(hitTypeToStatusMapEntry.second.second);
        std::cout << "Hit type " << hitType << ": " << nHitsInTPC << " hits in TPC, " << nHitsOutOfTPC << " hits out of TPC" << std::endl;
    }
