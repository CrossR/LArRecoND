...
{
    // Transportation
    processTable[1][91] = MC_PROC_TRANSPORTATION; // Maps "Transportation:Transportation"
    processTable[1][92] = MC_PROC_TRANSPORTATION; // Maps "Transportation:CoupledTransportation"

    // Electromagnetic
    processTable[2][1] = MC_PROC_COULOMB_SCAT; // Maps "EM:CoulombScattering"
    processTable[2][2] = MC_PROC_E_IONI; // Maps "EM:Ionisation"
    processTable[2][3] = MC_PROC_E_BREM; // Maps "EM:Bremsstrahlung"
    processTable[2][4] = MC_PROC_CONV; // Maps "EM:PairProductionByCharged"
    processTable[2][5] = MC_PROC_ANNIHIL; // Maps "EM:Annihilation"
    processTable[2][6] = MC_PROC_ANNIHIL; // Maps "EM:AnnihilationToMuMu"
    processTable[2][7] = MC_PROC_ANNIHIL; // Maps "EM:AnnihilationToHadrons"
    processTable[2][8] = MC_PROC_UNKNOWN; // Maps "EM:NuclearStopping"
    processTable[2][10] = MC_PROC_UNKNOWN; // Maps "EM:MultipleScattering"
    processTable[2][11] = MC_PROC_RAYLEIGH; // Maps "EM:Rayleigh"
    processTable[2][12] = MC_PROC_PHOT; // Maps "EM:PhotoElectricEffect"
    processTable[2][13] = MC_PROC_COMPT; // Maps "EM:ComptonScattering"
    processTable[2][14] = MC_PROC_CONV; // Maps "EM:GammaConversion"
    processTable[2][15] = MC_PROC_CONV; // Maps "EM:GammaConversionToMuMu"
    processTable[2][21] = MC_PROC_UNKNOWN; // Maps "EM:Cerenkov"
    processTable[2][22] = MC_PROC_UNKNOWN; // Maps "EM:Scintillation"
    processTable[2][23] = MC_PROC_UNKNOWN; // Maps "EM:SynchrotronRadiation"
    processTable[2][24] = MC_PROC_UNKNOWN; // Maps "EM:TransitionRadiation"

    // Optical
    processTable[4][31] = MC_PROC_UNKNOWN; // Maps "OPT:OpAbsorption"
    processTable[4][32] = MC_PROC_UNKNOWN; // Maps "OPT:OpBoundary"
    processTable[4][33] = MC_PROC_UNKNOWN; // Maps "OPT:OpRayleigh"
    processTable[4][34] = MC_PROC_UNKNOWN; // Maps "OPT:OpWLS"
    processTable[4][35] = MC_PROC_UNKNOWN; // Maps "OPT:OpMieHG"

    // Hadronic
    processTable[4][111] = MC_PROC_HAD_ELASTIC; // Maps "HAD:HadronElastic"
    processTable[4][121] = MC_PROC_NEUTRON_INELASTIC; // Maps "HAD:HadronInelastic"
    processTable[4][131] = MC_PROC_N_CAPTURE; // Maps "HAD:Capture"
    processTable[4][141] = MC_PROC_UNKNOWN; // Maps "HAD:Fission"
    processTable[4][151] = MC_PROC_UNKNOWN; // Maps "HAD:HadronAtRest"
    processTable[4][152] = MC_PROC_MU_MINUS_CAPTURE_AT_REST; // Maps "HAD:LeptonAtRest"
    processTable[4][161] = MC_PROC_UNKNOWN; // Maps "HAD:ChargeExchange"
    processTable[4][210] = MC_PROC_RADIOACTIVE_DECAY_BASE; // Maps "HAD:RadioactiveDecay"

    // Decay
    processTable[6][201] = MC_PROC_DECAY; // Maps "DEC:Decay"
    processTable[6][202] = MC_PROC_DECAY; // Maps "DEC:DecayWithSpin"
    processTable[6][203] = MC_PROC_DECAY; // Maps "DEC:DecayPionMakeSpin"
    processTable[6][210] = MC_PROC_RADIOACTIVE_DECAY_BASE; // Maps "DEC:Radioactive"
    processTable[6][211] = MC_PROC_DECAY; // Maps "DEC:Unknown"
    processTable[6][231] = MC_PROC_DECAY; // Maps "DEC:External"

    // General
    processTable[7][401] = MC_PROC_UNKNOWN; // Maps "General:StepLimiter"
    processTable[7][402] = MC_PROC_UNKNOWN; // Maps "General:UserSpecialCuts"
    processTable[7][403] = MC_PROC_NEUTRON_KILLER; // Maps "General:NeutronKiller"

    // UCN
    processTable[12][41] = MC_PROC_UNKNOWN; // Maps "UCN:UCNLoss"
    processTable[12][42] = MC_PROC_UNKNOWN; // Maps "UCN:UCNAbsorption"
    processTable[12][43] = MC_PROC_UNKNOWN; // Maps "UCN:UCNBoundary"
    processTable[12][44] = MC_PROC_UNKNOWN; // Maps "UCN:UCNMultiScattering"
}
...
void CreateSPMCParticles(const LArSPMC &larspmc, const pandora::Pandora *const pPrimaryPandora, const Parameters &parameters)
{
    ...
    // Fill the process table, for later use when creating MC particles.
    std::unordered_map<int, std::unordered_map<int, MCProcess>> processTable;
    FillProcessTable(processTable);
    ...
    for (size_t i = 0; i < larspmc.m_mcp_id->size(); ++i)
    {
        ...
        if (processTable.find(startProcess) == processTable.end() ||
            processTable[startProcess].find(startSubProcess) == processTable[startProcess].end())
            mcParticleParameters.m_process = MC_PROC_UNKNOWN;
        else
            mcParticleParameters.m_process = processTable[startProcess][startSubProcess];
